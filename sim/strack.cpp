...
void STrackSrc::update_rtt(simtime_picosec delay)
{
...
void STrackSrc::adjust_cwnd(simtime_picosec delay, STrackAck::seq_t ackno)
{
...
void STrackSrc::applySTrackLimits()
{
...
void STrackSrc::handle_ack(STrackAck::seq_t ackno)
{
    simtime_picosec now = eventlist().now();
...
bool STrackSrc::send_next_packet()
{
...
void STrackSrc::receivePacket(Packet &pkt)
{
...
void STrackSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period)
{
...
void STrackSrc::reroute(const Route &routeout)
{
    cout << "reroute!" << endl;
...
void STrackSrc::log(STrackLogger::STrackEvent event)
{
    if (_logger)
...
void STrackSrc::set_cwnd(uint32_t cwnd)
{
    _strack_cwnd = cwnd;
    _default_cwnd = cwnd;
}

void STrackSrc::set_hdiv(double hdiv)
{
    _h = _base_delay / hdiv; // path length scaling constant.  Value is a guess, will be clarified by experiment
}

void STrackSrc::set_paths(vector<const Route *> *rt_list)
{
    cout << "set_paths!" << endl;
...
void STrackSrc::connect(const Route &routeout, const Route &routeback, STrackSink &sink,
...
void STrackPacer::schedule_send(simtime_picosec delay)
{
...
void STrackPacer::just_sent()
{
    _last_send = eventlist().now();
}

void STrackPacer::doNextEvent()
{
...
void STrackSink::connect(STrackSrc &src, const Route &route_back)
{
    _src = &src;
...
void STrackSink::receivePacket(Packet &pkt)
{
