...
void SwiftSubflowSrc::applySwiftLimits()
{
...
void SwiftSubflowSrc::handle_ack(SwiftAck::seq_t ackno)
{
    simtime_picosec now = eventlist().now();
    ...
    {                                      // a brand new ack
        ...
        {
            cout << "Flow " << _src._name << " " << _src.get_id() << " finished at "
                 << timeAsUs(eventlist().now())
                 << " total bytes " << _last_acked << endl;
        }
...
int SwiftSubflowSrc::send_packets()
{
...
bool SwiftSubflowSrc::send_next_packet()
{
...
void SwiftSubflowSrc::send_callback()
{
...
void SwiftSubflowSrc::receivePacket(Packet &pkt)
{
...
void SwiftSubflowSrc::connect(SwiftSink &sink, const Route &routeout, const Route &routeback, uint32_t flow_id, BaseScheduler *scheduler)
{
...
void SwiftSubflowSrc::reroute(const Route &routeout)
{
...
void SwiftSrc::log(SwiftSubflowSrc *sub, SwiftLogger::SwiftEvent event)
{
    if (_logger)
...
void SwiftSrc::connect(const Route &routeout, const Route &routeback, SwiftSink &sink,
...
void SwiftSubflowSink::receivePacket(Packet &pkt)
{
...
void SwiftSink::receivePacket(Packet &pkt)
{
    ...
    if (_src && _cumulative_data_ack >= _src->_flow_size)
    {
        cout << "Flow " << _src->_name << " " << _src->get_id() << " finished at "
             << timeAsUs(_src->eventlist().now())
             << " total bytes " << _cumulative_data_ack << endl;
    }
}
